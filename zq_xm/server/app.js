...
// 获取导航栏数据的接口
server.get('/compunent', (req, res) => {
  var obj=req.query
  // SQL语句以获取文章分类表的数据
  let sql = 'select title,title2,title3 from zq_emp where eid=?';
  // 执行SQL语句
  pool.query(sql, [obj.lid],(error, results) => {
...
server.get('/compunents', (req, res) => {
  ...
  let rowcount;
  // 获取指定分类下的文章总数
  let sql = 'SELECT COUNT(reid) AS count FROM zq_remp WHERE ruid=?';
  ...
  pool.query(sql, [cid], (error, results) => {
    ...
    // 查询SQL语句
    sql = 'SELECT * FROM zq_remp WHERE ruid=? LIMIT ?,?';
    // 执行SQL
